...
export default function AgingRecordTable({ searchTerm = "" }: Props) {
  ...
  const fetchAgingRecords = async () => {
    try {
      setLoading(true);
      const { data: agingData, error: fetchError } = await supabase.from("aging_process").select("*").order("aging_date", { ascending: false });
      if (fetchError) { setError(fetchError.message); } else { setData(agingData || []); setFilteredData(agingData || []); }
  ...
  const handleExport = () => {
    exportToCSV(filteredData, "aging_process", ["aging_date", "selected_buckets", "bucket_count", "total_weight", "batch_in_time", "batch_out_time"]);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h4 className="text-lg font-semibold text-slate-900 dark:text-white">Aging Process Records</h4><p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Total records: {filteredData.length} {data.length !== filteredData.length && `(of ${data.length})`}</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={fetchAgingRecords} className="px-3 py-1.5 text-sm font-medium rounded-lg bg-slate-700 hover:bg-slate-600 text-white transition-colors">Refresh</button>
      ...
      {filteredData.length === 0 ? (<div className="rounded-xl border border-slate-200 dark:border-slate-700 p-8 text-center"><p className="text-slate-500 dark:text-slate-400">{data.length === 0 ? "No aging process records found." : "No records match the selected date range."}</p></div>) : (
          ...
          <div className="overflow-x-auto overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600 scrollbar-track-slate-100 dark:scrollbar-track-slate-800">
            <table className="w-full">
              <thead className="sticky top-0 z-10 bg-slate-800">
                <tr>{["Aging Date", "Selected Buckets", "Bucket Count", "Total Weight (kg)", "Batch In Time", "Batch Out Time"].map((col) => (<th key={col} className="px-3 py-3 text-left text-xs font-bold uppercase tracking-wider text-white whitespace-nowrap border-b-2 border-slate-600">{col}</th>))}</tr>
              </thead>
