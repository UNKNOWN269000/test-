...
export default function AnodizingBarBindingTable({ searchTerm = "" }: Props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: d, error: e } = await supabase
        ...
        .select("*")
        .order("extrusion_date", { ascending: false });
  ...
  const handleExport = () => {
    exportToCSV(filteredData, "anodizing_binding", [
      "extrusion_date", "billet_batch", "die_no", "profile", "bucket_no",
      "surface", "full_rack_no", "one_full_rack_qty", "pcs_rack_no", "pcs_qty",
      "surface2", "full_rack_no2", "one_full_rack_qty2", "pcs_rack_no2", "pcs_qty2",
      "total_binding_qty", "type", "binding_team", "average_time", "rejection_qty"
    ]);
  };
