...
export default function PowdercoatPackingTable({ searchTerm = "" }: Props) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data: d, error: e } = await supabase.from("powder_coat_packing").select("*").order("packing_date", { ascending: false });
  ...
  const handleExport = () => {
    const exportData = filteredData.map((row) => ({
      ...
      profile: row.profile,
      colour: row.colour,
      re_powder_qty: row.re_powder_qty,
      ...
      weightbar_one_qty: row.weightbar_enabled ? row.weightbar_one_qty : null,
      weightbar_avg_weight: row.weightbar_enabled ? row.weightbar_avg_weight : null,
    }));
    exportToCSV(exportData, "powder_coat_packing");
  };

  if (loading) return <HiTechLoader text="Loading Powder Coat Packing" />;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h4 className="text-lg font-semibold text-slate-900 dark:text-white">Powder Coat Packing Records</h4><p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Total records: {filteredData.length} {data.length !== filteredData.length && `(of ${data.length})`}</p></div>
        <DateFilter />
      </div>
          ...
          <div className="overflow-x-auto overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600 scrollbar-track-slate-100 dark:scrollbar-track-slate-800">
            <table className="w-full text-xs">
              <thead className="sticky top-0 z-20 bg-slate-800">
                <tr>
                  <th colSpan={8} className="px-2 py-2 text-center text-xs font-bold uppercase tracking-wider text-white border-r border-slate-600">Basic Info</th>
                ...
                <tr>{["Production Date", "Packing Date", "Bucket No", "Length", "Type", "Profile", "Colour", "Re-Powder Qty", "Pack No", "1 Qty", "Total Bundles", "Total Qty", "Pack No", "1 Qty", "Total Qty", "Avg Wt", "Pack No", "1 Qty", "Total Bundles", "Total Qty", "Pack No", "1 Qty", "Total Qty", "Avg Wt", "Pack No", "1 Qty", "Avg Wt"].map((col, idx) => (<th key={idx} className="px-2 py-2 text-left text-xs font-bold uppercase tracking-wider text-white whitespace-nowrap border-r border-slate-600">{col}</th>))}</tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {filteredData.map((row, idx) => (<tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-2 py-2 whitespace-nowrap border-r border-slate-200 dark:border-slate-700"><span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: `${row.colour}20`, color: row.colour, border: `1px solid ${row.colour}40` }}>{row.colour}</span></td>
                  <td className="px-2 py-2 text-slate-700 dark:text-slate-300 whitespace-nowrap border-r border-slate-200 dark:border-slate-700">{row.re_powder_qty || "—"}</td>
                  ...
                  <td className="px-2 py-2 text-slate-700 dark:text-slate-300 whitespace-nowrap border-r border-slate-200 dark:border-slate-700">{row.weightbar_enabled ? (row.weightbar_one_qty || "—") : "—"}</td>
